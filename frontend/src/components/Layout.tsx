...
export default function Layout() {
  ...
  const [chatModel, setChatModel] = useState('模型读取中...');

  useEffect(() => {
    systemApi.getRuntimeInfo()
      .then((runtimeInfo) => {
        setChatModel(runtimeInfo.chatModel || '模型未配置');
      })
      .catch(() => {
        setChatModel('模型未配置');
      });
  ...
  const navGroups: NavGroup[] = [
    {
      ...
      items: [
        ...
        { id: 'job-target', path: '/job-target', label: '岗位目标', icon: Target, description: 'JD 解析、管理与匹配' },
  ...
  const isActive = (path: string) => {
    ...
    if (path === '/job-target') {
      return currentPath.startsWith('/job-target') || currentPath.startsWith('/job-descriptions');
    }
      ...
      <aside className="w-64 bg-white border-r border-slate-100 fixed h-screen left-0 top-0 z-50 flex flex-col">
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="px-3 py-2 bg-gradient-to-r from-primary-50 to-indigo-50 rounded-xl">
            <p className="text-xs text-primary-600 font-medium">AI 面试助手 v1.2</p>
            <p className="text-xs text-slate-500 mt-0.5 truncate" title={chatModel}>
              模型: {chatModel}
            </p>
          </div>
        </div>
