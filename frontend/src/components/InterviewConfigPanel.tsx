    ...
    >
      <div className="bg-white rounded-2xl p-8 shadow-sm">
        ...
        <div className="space-y-6">
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-slate-600 mb-3">可选岗位 JD（可跳过）</label>
            <select
              value={selectedJdId ?? ''}
              onChange={(e) => onSelectJd(e.target.value ? Number(e.target.value) : null)}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-700"
            >
              <option value="">不选择 JD（通用面试）</option>
              {jdOptions.map((jd) => (
                <option key={jd.id} value={jd.id}>{jd.jobTitle}{jd.companyName ? ` · ${jd.companyName}` : ''}</option>
              ))}
            </select>
          </div>
