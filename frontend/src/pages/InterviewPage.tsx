...
interface InterviewProps {
  resumeText: string;
  resumeId?: number;
  jobContext?: CreateInterviewRequest['jobContext'];
  onBack: () => void;
  onInterviewComplete: () => void;
}

export default function Interview({ resumeText, resumeId, jobContext, onBack, onInterviewComplete }: InterviewProps) {
  ...
  const [selectedJdId, setSelectedJdId] = useState<number | null>(jobContext?.jdId ?? null);
  const [selectedJobContext, setSelectedJobContext] = useState<CreateInterviewRequest['jobContext'] | undefined>(jobContext);
  ...
  useEffect(() => {
    jobMatchingApi.listJobDescriptions()
      .then((list) => setJdOptions(list.filter((i) => i.parseStatus === 'COMPLETED')))
      .catch(() => setJdOptions([]));
  }, []);
  ...
  useEffect(() => {
    if (!selectedJdId) {
      if (!jobContext?.jdId) {
        setSelectedJobContext(undefined);
      }
      return;
    }
    const selected = jdOptions.find((item) => item.id === selectedJdId);
    if (!selected) return;
    setSelectedJobContext({
      jdId: selected.id,
      jobTitle: selected.jobTitle,
      jdSummary: selected.coreRequirements || selected.responsibilities || '',
      requiredSkills: selected.technicalSkills || [],
      focusSkills: selected.technicalSkills || [],
    });
  }, [selectedJdId, jdOptions, jobContext]);
