...
import { historyApi, InterviewHistoryItem, EvaluateStatus } from '../api/history';
...
interface InterviewWithResume extends InterviewHistoryItem {
...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview }: InterviewHistoryPageProps) {
  ...
  const loadAllInterviews = useCallback(async (isPolling = false) => {
    ...
    try {
      const allInterviews = await historyApi.getInterviewHistory();
  ...
  const filteredInterviews = interviews.filter(interview =>
    `${interview.resumeFilename || ''} ${interview.jobTitle || ''}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        >
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="text-left px-6 py-4 text-sm font-medium text-slate-600">关联上下文</th>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-slate-800">{interview.resumeFilename || '无简历'}</p>
                          <p className="text-xs text-slate-400">JD: {interview.jobTitle || '无JD'} · 简历: {interview.resumeFilename || '无简历'}</p>
