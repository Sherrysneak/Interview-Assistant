...
public interface InterviewMapper {
    ...
    default java.util.Map<String, Object> toInterviewHistoryItem(InterviewSessionEntity session) {
        ...
        map.put("resumeId", session.getResume() != null ? session.getResume().getId() : null);
        map.put("resumeFilename", session.getResume() != null ? session.getResume().getOriginalFilename() : session.getResumeFilenameSnapshot());
        map.put("jdId", session.getJobDescription() != null ? session.getJobDescription().getId() : null);
        map.put("jobTitle", session.getJobDescription() != null ? session.getJobDescription().getJobTitle() : session.getJobTitleSnapshot());
        map.put("sourceType", session.getSourceType() != null ? session.getSourceType().name() : InterviewSessionEntity.SessionSourceType.LEGACY.name());
        map.put("hasResume", session.getResume() != null);
        map.put("hasJd", session.getJobDescription() != null);
        return map;
    }
