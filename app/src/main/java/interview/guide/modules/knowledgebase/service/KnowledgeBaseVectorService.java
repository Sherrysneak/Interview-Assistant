...
public class KnowledgeBaseVectorService {
    ...
    public List<Document> similaritySearch(String query, List<Long> knowledgeBaseIds, int topK, double minScore) {
        ...
        try {
            ...
            // 即使向量库端已做过滤，也在服务层做一次兜底，保证行为一致。
            results = applyLocalFilterAndLimit(results, knowledgeBaseIds, topK);
    ...
    private List<Document> similaritySearchFallback(String query, List<Long> knowledgeBaseIds, int topK, double minScore) {
        try {
            ...
            List<Document> results = applyLocalFilterAndLimit(allResults, knowledgeBaseIds, topK);
    ...
    private List<Document> applyLocalFilterAndLimit(List<Document> source, List<Long> knowledgeBaseIds, int topK) {
        List<Document> filtered = source;
        if (knowledgeBaseIds != null && !knowledgeBaseIds.isEmpty()) {
