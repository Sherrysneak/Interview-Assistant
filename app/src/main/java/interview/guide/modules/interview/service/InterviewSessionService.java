...
public class InterviewSessionService {
    ...
    public InterviewSessionDTO createSession(CreateInterviewRequest request) {
        String normalizedResumeText = normalizeResumeText(request);
        ...
        // 保存到数据库（支持 resumeId 为空的场景，保证融合历史完整）
        try {
            persistenceService.saveSession(sessionId, request.resumeId(),
                normalizedResumeText, questions.size(), questions, request.jobContext());
        } catch (Exception e) {
    ...
    private CachedSession restoreSessionFromEntity(InterviewSessionEntity entity) {
        try {
            ...
            sessionCache.saveSession(
                entity.getSessionId(),
                entity.getResumeTextSnapshot() != null ? entity.getResumeTextSnapshot() : "",
    ...
    private String normalizeResumeText(CreateInterviewRequest request) {
        String text = request.resumeText();
        if (text != null && !text.isBlank()) {
            return text;
        }

        if (request.resumeId() != null) {
            return resumeRepository.findById(request.resumeId())
                .map(r -> r.getResumeText() == null ? "" : r.getResumeText())
                .orElse("");
        }

        if (request.jobContext() != null) {
            String summary = request.jobContext().jdSummary();
            if (summary != null && !summary.isBlank()) {
                return "候选人未提供简历。岗位上下文：" + summary;
            }
            String jobTitle = request.jobContext().jobTitle();
            if (jobTitle != null && !jobTitle.isBlank()) {
                return "候选人未提供简历。目标岗位：" + jobTitle;
            }
        }

        return "候选人未提供简历，请按通用技术面试流程进行提问。";
    }
}
