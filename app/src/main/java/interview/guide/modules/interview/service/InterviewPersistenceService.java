...
public class InterviewPersistenceService {
    ...
    public InterviewSessionEntity saveSession(
        ...
        try {
            ...
            session.setResumeTextSnapshot(resumeText);
            session.setResumeFilenameSnapshot(resume != null ? resume.getOriginalFilename() : null);
            if (jobContext != null) {
                session.setContextSnapshotJson(objectMapper.writeValueAsString(jobContext));
                session.setJobTitleSnapshot(jobContext.jobTitle());
                if (jobContext.jdId() != null) {
                    jobDescriptionRepository.findByIdAndDeletedAtIsNull(jobContext.jdId())
                        .ifPresent(session::setJobDescription);
                }
            }

            if (session.getResume() != null && session.getJobDescription() != null) {
                session.setSourceType(InterviewSessionEntity.SessionSourceType.FUSION_ENTRY);
            } else if (session.getJobDescription() != null) {
                session.setSourceType(InterviewSessionEntity.SessionSourceType.JD_ENTRY);
            } else if (session.getResume() != null) {
                session.setSourceType(InterviewSessionEntity.SessionSourceType.RESUME_ENTRY);
            }
            
            InterviewSessionEntity saved = sessionRepository.save(session);
            log.info("面试会话已保存: sessionId={}, resumeId={}, jdId={}",
                sessionId,
                resumeId,
                session.getJobDescription() != null ? session.getJobDescription().getId() : null);
     ...
     * 获取全部面试历史（含上下文关联）。
     */
    public List<InterviewSessionEntity> findAllWithContext() {
        return sessionRepository.findAllWithContextOrderByCreatedAtDesc();
    }
