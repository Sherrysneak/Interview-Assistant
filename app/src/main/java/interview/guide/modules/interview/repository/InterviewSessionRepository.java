...
public interface InterviewSessionRepository extends JpaRepository<InterviewSessionEntity, Long> {
     ...
     * 根据会话ID查找（同时加载关联的简历）
     */
    @Query("SELECT s FROM InterviewSessionEntity s LEFT JOIN FETCH s.resume LEFT JOIN FETCH s.jobDescription WHERE s.sessionId = :sessionId")
    Optional<InterviewSessionEntity> findBySessionIdWithResume(@Param("sessionId") String sessionId);

    /**
     * 查询全量面试历史，附带简历与JD上下文。
     */
    @Query("SELECT s FROM InterviewSessionEntity s LEFT JOIN FETCH s.resume LEFT JOIN FETCH s.jobDescription ORDER BY s.createdAt DESC")
    List<InterviewSessionEntity> findAllWithContextOrderByCreatedAtDesc();
